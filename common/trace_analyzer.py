...
class TraceAnalyzer:
    def __init__(self, max_duration=40):
        ...
        self.all_stats = {"EgoCutInTracer": [], "EgoCutInSideTracer": [], "EgoCutOutTracer": [], "CutInTracer": [], "CutInSideTracer": [],  "FrontSlowDownSameLaneTracer": []}
    ...
    def get_safe_distance(self, v_lead: float, v_follow: float) -> float:        
        ...
        safe_distance = (v_lead**2 - v_follow**2) / (
            2 * deceleration
        ) + v_follow * reaction_time + min_distance
        return safe_distance
    

    
    def time_to_collision(self, v_lead: float, v_follow: float, distance: float) -> float:
        """
    ...
    def analyze(self, trace_dict: dict, all_frames_dict: dict, crashed=True) -> None:
        ...
        for step in reversed(trace_dict):
            for tracer in trace_dict[step]:
                if tracer in ["CutInTracer", "FrontSlowDownSameLaneTracer"]and trace_dict[step][tracer] == False:
                    try:
                        if trace_dict[step+1][tracer] == True:
        ...
        if event_step is not None:
            ...
            if event_frame["ego_speed"] > event_frame["adv_speed"]:
                ...
                ttc = self.time_to_collision(
                    event_frame["ego_speed"], event_frame["adv_speed"], event_frame["adv_ego_distance"]
                )
                observed_distance = event_frame["adv_ego_distance"]
                print(f"Event detected {tracer_found} at step {event_step} with safe distance {safe_distance} and observed distance {observed_distance}")
                ...
                if ego_cut_in and observed_distance < safe_distance: # TODO: consider a case where ego cut in and adversry cut in happen at the same time
        ...
        ego_side_cut_in = [trace_dict[step]["EgoCutInSideTracer"] for step in trace_dict]
        ...
        adv_crash_behind = last_frame["adv_ego_distance"] < -4.9 and abs(last_frame["adv_heading"]) < 0.1 

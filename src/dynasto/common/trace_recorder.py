...
class TraceRecorder:
    ...
    def update_trace(
        self,
        ego_veh: ControlledVehicle,
        ego_action: int,
        adv_veh: ControlledVehicle,
        adv_action: int,
    ):
        """
        ...
        self.current_frame_dict["ego_acceleration"] = float(
            ego_veh.action["acceleration"]
        )
        ...
        self.current_frame_dict["adv_acceleration"] = float(
            adv_veh.action["acceleration"]
        )
        self.current_frame_dict["adv_heading"] = float(adv_veh.heading)
        self.current_frame_dict["adv_target_lane"] = adv_target_lane
        self.current_frame_dict["adv_ego_distance"] = float(
            adv_veh.position[0] - ego_veh.position[0]
        )
        self.all_frames_dict[self.current_frame] = (
            self.current_frame_dict.copy()
        )  # Store a copy of the current frame data
    ...
    def save_trace(self, save_path=None):
        ...
        trace_file = os.path.join(
            save_folder, f"scenario_trace_episode_recording_{self.episode}.json"
        )
    ...
    def save_trace_init(self, ego: ControlledVehicle, adv: ControlledVehicle):
        ...
        scenario_file = os.path.join(
            self.save_folder, f"scenario_init_episode_{self.episode}.json"
        )

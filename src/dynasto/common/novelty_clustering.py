...
def get_clusters_from_distance_matrix(
    dist_matrix: np.ndarray, threshold: float = 0.2, linkage: str = "single"
) -> np.ndarray:
...
def find_clusters(
    ...
    cluster_num = get_clusters_from_distance_matrix(
        distance_matrix, threshold=threshold, linkage=linkage
    )
    if plot:
        plot_linkage_matrix(
            distance_matrix, threshold=threshold, method=linkage, save_path=save_path
        )
...
def plot_linkage_matrix(distances, method="ward", save_path=None, threshold=0):
    ...
    dendrogram(
        linkage_matrix, leaf_rotation=90, leaf_font_size=8, color_threshold=threshold
    )
